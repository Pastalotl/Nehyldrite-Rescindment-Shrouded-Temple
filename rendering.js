...
const actionBarY = -75
const leftActionStart = -145
const actionPromptsXOffset = -10
const actionPromptsYOffset = -8

function interfaceRendering(){ /*draws all the non-world elements such as menus and icons*/
  temple.renderDamageReads()
  /*Turn Order:*/
  //console.log(temple)
    ...
    turnOrderText.writeText(`${temple.peek().getType()} [${temple.peek().getAllegiance().toUpperCase()}]|turn:${temple.getPriority()}|[C]: end turn`)
    ...
    for(x=0;x<leftActionCount;x++){
      ...
      keyPrompts.writeGlyph(String(x+1),leftActionStart+x*actionWidth+actionPromptsXOffset,actionBarY+actionPromptsYOffset)
    ...
    if(lowerPortrait>6){lowerPortrait = 5} /*default to Vayens if everything goes wrong (lol)
    this is just in case other characters get their allegiance changed to make them playable
    and therefore lack the assets*/
    ...
    charName.writeText(menuCharacter.getType())
    //charName.writeText(menuNames[lowerPortrait])

    //console.log((5-frame)/5)
    healthBar.setPercentage(menuCharacter.getHealthPercent())
    healthBar.render()
    menuHealthValue.writeText(String(Math.round(menuCharacter.getCurrentHealth()))+"/"+String(menuCharacter.getMaxHealth()))

    skillBar.rowChange(lowerPortrait)
    skillBar.setFrame(menuCharacter.getMeter())
    skillBar.render()
    //if(actionNum==-1||actionNum>5){ /*make sure it doesn't get too busy*/
      skillText.writeText(skillNames[lowerPortrait])
    //}
    ...
    for(n=rightActionCount-1;n>-1;n--){
      ...
      menu.render()
      keyPrompts.writeGlyph(String((n+5)%10),rightActionStart+(n-rightActionCount)*actionWidth+actionPromptsXOffset,actionBarY+actionPromptsYOffset)
      menu.frameIncrement(-1)
    }
    if(actionNum!=-1){
...
const charPromptXOffset = -9
const charPromptYOffset = -9

function startScreenRendering(){
  textVersion.writeText("v0.3.9a (27/07/26)")
  ...
  if(startSelectMenu && !started){
    ...
    for(let n=0;n<portraitCount;n++){
      ...
      keyPrompts.writeGlyph(String(n+1),portraitStartX + (n%2) * portraitTesselationWidth+charPromptXOffset,portraitDefaultY - n*portraitTesselationHeight+charPromptYOffset)
    }
  }
}
